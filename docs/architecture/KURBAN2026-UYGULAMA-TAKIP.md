...
**Durum:** Başladı — Platform Süper Admin 360° operasyon uygulaması uygulandı; genel doğrulama ve kalan Faz 2B sertleştirmeleri bekliyor.
...
| Faz 2B platform kimlik ve oturum | Uygulandı — genel doğrulama bekliyor | Ayrı PlatformUser parola doğrulama, DB session, host-only cookie, rotation/logout/revoke, kullanıcı/rol authVersion iptali, başarısız giriş audit’i, kontrollü kilit, zorunlu TOTP MFA ve açıkça yetkilendirilmiş ilk Süper Admin bootstrap CLI uygulandı. Tam WebAuthn/passkey ve recovery uygulanmadı. |
...
2B integration test kapsamı `packages/database-platform/tests/platform-postgres.integration.test.ts` içindedir. Testler `RUN_PLATFORM_POSTGRES_TESTS=1` ve `PLATFORM_TEST_DATABASE_URL` olmadan normal unit koşusunda atlanır; CI'da geçici PostgreSQL servisiyle çalışır. Kapsam: boş DB’ye `0001..0006`, eski migration sonrası güncel zincir, drift/idempotent deploy, check constraint, foreign key, unique, gerçek repository create/read/nested write, transaction commit/rollback ve connection string sızıntısı kontrolü. Platform Admin build’i ayrıca CI kapısındadır. Faz 2C iki firma web/pool/backup testi ayrı tenant PostgreSQL servisinde `RUN_TENANT_ISOLATION_TESTS=1` ile çalışır.

Bu kayıt Faz 2B'nin bütün profesyonel genişletmelerinin kapandığı veya canlıya hazır olduğu anlamına gelmez. Tam WebAuthn/passkey/recovery, emergency-stop/incident/bakım davranışları, firma yapılandırma karşılaştırması, export/devir, canlı DNS/TLS/deployment, production restore onayı ve genel güvenlik/E2E kabulü kalır.
...
|---|---|---|---|---|---|
| 1 | Faz 2B | Platform domain, platform PostgreSQL `0001..0006`, repository adaptörleri, ayrı Platform Admin host/app, parola+TOTP MFA, DB session/cookie, dört rol ve API yetkisi, komuta merkezi, firma 360°, async provisioning, plan/lisans, domain, backup, SupportSession, kullanıcı ve audit akışları. | Firma Admin hazırlığı PlatformUser’dan ayrılıp `TenantAdminInvitation` oldu; komut kuyruğu gerçek provisioning ve tenant-ops worker’larına bağlandı. | Incident, maintenance, emergency-stop ve support ticket link modellerinin bazıları hâlâ metadata düzeyinde; passkey modeli hazır ama doğrulama akışı yok. | Tam WebAuthn/passkey/recovery, emergency-stop/incident/bakım davranışı, yapılandırma karşılaştırma, export/devir, canlı DNS/TLS/deployment, production restore onayı ve genel E2E/güvenlik kabulü. |
