...
| Faz 2C tenant-aware runtime ve pool | Uygulandı — genel doğrulama bekliyor | Host/custom-domain resolution ile session/databaseRef guard korunarak tenant+DB ref sahiplikli ayrı Prisma pool’ları, eşzamanlı güvenli reuse, idle kapatma ve shutdown temizliği eklendi. Mevcut tenant-web route’ları henüz bu runtime’a taşınmadı; DNS/TLS/deployment yapılmadı. |
...
| Faz 2C iki firma izolasyon otomasyonu | CI kapısına bağlandı | İki ayrı organization/DB, aynı kayıt ID’leri, host/session/ref fail-closed davranışı, ayrı Prisma pool, SupportSession sınırı, güvenli hata payload’ı ve üçüncü tenant rollback izolasyonu gerçek PostgreSQL üzerinde doğrulanır. |

2B-1B integration test kapsamı `packages/database-platform/tests/platform-postgres.integration.test.ts` içindedir. Testler `RUN_PLATFORM_POSTGRES_TESTS=1` ve `PLATFORM_TEST_DATABASE_URL` olmadan normal unit koşusunda atlanır; CI'da geçici PostgreSQL servisiyle çalışır. Kapsam: boş DB’ye `0001..0004`, eski migration sonrası güncel zincir, drift/idempotent deploy, check constraint, foreign key, unique, gerçek repository create/read/nested write, transaction commit/rollback ve connection string sızıntısı kontrolü. Faz 2C iki firma testi ayrı tenant PostgreSQL servisinde `RUN_TENANT_ISOLATION_TESTS=1` ile çalışır.
...
|---|---|---|---|---|---|
| 1 | Faz 2B | Platform domain, platform PostgreSQL şeması, `0001..0004` migration zinciri, repository port/adaptörleri, gerçek PostgreSQL CI testi, platform kullanıcı/rol temeli, control-plane metadata ve devam ettirilebilir provisioning job kaydı. | Provisioning iş durumu gerçek platform repository adapterine bağlandı. | MFA/passkey, session/device, incident, maintenance, support ticket linkleri metadata düzeyinde. | Süper Admin UI, platform login/session, gerçek MFA/passkey doğrulama, provisioning durum ekranı, firma/modül emergency stop davranışı. |
| 2 | Faz 2C | Gerçek tenant DB create/exists/migrate/verify/rollback adapteri, ownership marker, idempotent/resumable provisioning, kontrollü CLI, host/custom-domain resolution, session/databaseRef fail-closed guard, tenant-aware Prisma pool ve iki firma PostgreSQL isolation CI testi. | Gerçek DB admin adapteri ve connection pool sözleşme düzeyinden çalışan altyapıya taşındı. | Mevcut tenant-web henüz yeni runtime/pool’a bağlı değil. | Tenant-web runtime bağlama, backup/restore izolasyon otomasyonu, WAL/PITR değerlendirmesi, pool kapasite/metric politikası ve canlı DNS/TLS/deployment. |
