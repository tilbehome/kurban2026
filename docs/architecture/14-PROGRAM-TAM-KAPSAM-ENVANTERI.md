...
| `Vekalet` / `ProxyGrant` | Hisse bazlı belge | `IMPLEMENTED_UNVERIFIED` | Çoklu veren, yöntem, durum geçmişi, belge snapshot, güvenli dosya portu ve süreli/iptal edilebilir QR kodlandı; gerçek kanal ve fiziksel baskı kabulü yok. | Faz 12 genel ve fiziksel kabul. | P1 |
...
| Vekâlet ve belge | `/hayvanlar/vekalet`, müşteri/hisse vekâlet tabları | `/api/vekaletler`, `/api/vekaletler/[id]`, `/api/hisseler/[id]/vekalet` | `ProxyGrant`, `ProxyGrantor`, belge/QR kayıtları | `IMPLEMENTED_UNVERIFIED` | Çoklu veren, yöntem, durum geçmişi, belge snapshot, metadata-only okuma ve kontrollü dosya çözümleme kodlandı; gerçek sağlayıcı/A4 kabulü `NOT_RUN`. | Faz 12 doğrulama ve fiziksel belge kabulü. | Faz 7/12 | P1 | Dosya public sızmaz; yetkisiz erişim yok. |
| Kesim, tartım, paket, teslim | `/kesim/*`, `/saha`, `/takip/[token]`, `/tv/*` | `/api/kesim/*`, tartım/paket/teslim ve takip API'leri | Kesim görev/istasyon/ekip, tartım, paket, teslim ve geçmiş modelleri | `IMPLEMENTED_UNVERIFIED` | Kilitli durum geçişleri, istisna, tartım geçmişi, paket izlenebilirliği, teslim checklist/geri alma ve PII'siz takip kodlandı; fiziksel cihaz/E2E yok. | Faz 12 genel, cihaz ve saha kabulü. | Faz 8–10/12 | P1 | Yetkisiz geçiş ve ikinci teslim engellenir; fiziksel senkron kabulü ayrıca yapılır. |
| Raporlar ve yazdırma | `/raporlar/*`, yönetim görünümü | `/api/raporlar/*`, yönetim rapor API'leri | Ledger ve operasyon read-model'leri | `IMPLEMENTED_UNVERIFIED` | Finans/operasyon/sezon karşılaştırma ve istisna read-model'leri ile formül enjeksiyonuna kapalı export kodlandı; mutabakat testleri `NOT_RUN`. | Faz 12 mutabakat ve çıktı kabulü. | Faz 11/12 | P2 | Raporlar ledger ve operasyon kayıtlarıyla tutarlı olur. |
...
| TV takip ve müşteri takip | `/tv`, `/tv/m`, `/takip/[token]` | `/api/tv/*`, güvenli takip API'si | Operasyon read-model'i ve süreli takip token'ı | `IMPLEMENTED_UNVERIFIED` | Public takip PII'siz, süreli ve iptal edilebilir olarak kodlandı; gerçek TV/mobil cihaz kabulü `NOT_RUN`. | Fiziksel ekran, rate-limit ve HTTPS kabulü. | Faz 8/10/12 | P1 | Müşteri yalnız token ile izinli ve PII'siz takip bilgisini görür. |
...
| Personel ve ekip | `/personel/*`, `/tv/personel`, kesim komuta merkezi | kesim görev/ekip/istasyon API'leri | Kesim görevi, ekip ve istasyon modelleri | `IMPLEMENTED_UNVERIFIED` (kesim operasyonu) | Kesim ekip/istasyon/görev ataması kodlandı; genel personel, vardiya ve performans ürünü tamamlanmış değildir. | Genel personel kapsamı planlı; kesim görevi Faz 12'de doğrulanır. | Faz 8/12/14 | P2 | Personel görevleri rol ve operasyon durumuyla bağlıdır. |
| Lojistik ve adrese teslim | `/lojistik/*`, teslimat yönetimi | teslimat API'leri | Teslim türü, hedef snapshot, paket checklist ve kanıt metadata | `IMPLEMENTED_UNVERIFIED` (teslim çekirdeği) | Çiftlik/adres teslim, kısmi teslim istisnası ve geri alma kodlandı; rota, araç ve şoför optimizasyonu planlıdır. | Rota/araç kapsamı ve fiziksel teslim kabulü. | Faz 10/12 | P2 | Teslimat tek seferlik ve auditli kapanır. |
...
| PWA/offline/service worker | `/offline`, `/saha`, PWA bileşenleri | offline sync ve audit API'leri | IndexedDB kuyruğu; tenant/sezon/kullanıcı/oturum/cihaz bağları | `IMPLEMENTED_UNVERIFIED` | Retry/backoff/poison/conflict ve fail-closed işlem sınıfları saha yüzeyine bağlandı; fiziksel zayıf ağ/cihaz kabulü `NOT_RUN`. | Faz 12 mobil E2E ve fiziksel cihaz kabulü. | Faz 10/12 | P1 | Zayıf ağda yalnız izinli işlemler güvenli davranır. |
