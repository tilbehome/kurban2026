...
| Hisse atama/satış | `/hayvanlar/hisse-atama`, `/saha-satis` | `/api/hisseler/ata`, `/api/hisseler/toplu-ata`, `/api/saha-satis` | `Hisse`, `Musteri`, `Odeme`, `KasaHareketi`, `IslemAnahtari` | Mevcut ve kısmi çalışıyor | Faz 1 pilot hata standardı var; tam domain/use-case ayrımı yok. | İnce route, domain kuralı, idempotent satış. | Faz 1/2/5 | P0 | Çifte satış olmaz; hata kodları standart; testler geçer. |
...
| Platform/Süper Admin | `/ayarlar/saas` | Yok | Yok | Gereksinimde var fakat kodlanmamış | Platform DB, platform IAM ve destek erişimi yok. | Ayrı platform uygulaması/veritabanı/kimlik. | Faz 6+ | P1 | Firma verisi platform DB'ye taşmaz. |
