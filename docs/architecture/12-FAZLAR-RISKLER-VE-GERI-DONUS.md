...
Faz 2B/2C uygulama notu: `@tilbecore/provisioning`, `@tilbecore/database-tenant` ve `@tilbecore/tenant-runtime` birlikte gerçek DB create/migrate/verify, kalıcı iş durumu, idempotent resume, sahiplik kanıtlı rollback ve tenant-aware pool akışını uygular. `apps/provisioning-cli` bu use-case’i kontrollü komutlarla çalıştırır; iki firma PostgreSQL isolation testi CI kapısındadır. Süper Admin provisioning ekranı Faz 2B’de; tenant-web runtime bağlama, backup/restore izolasyonu, WAL/PITR ve canlı deployment Faz 2C/sonraki operasyon paketlerinde kalır.
