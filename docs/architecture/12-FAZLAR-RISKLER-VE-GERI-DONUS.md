...
Faz 2B/2C uygulama notu: `@tilbecore/provisioning`, `@tilbecore/database-tenant`, `@tilbecore/tenant-runtime` ve `@tilbecore/tenant-web-runtime` birlikte gerçek DB create/migrate/verify, kalıcı iş durumu, idempotent resume, sahiplik kanıtlı rollback, request-local tenant bağlama, auditli SupportSession ve gözlemlenebilir tenant pool akışını uygular. `apps/provisioning-cli` provisioning; `apps/tenant-ops-cli` firma bazlı dump, checksum, geçici restore doğrulaması ve destructive olmayan restore planını kontrollü komutlarla çalıştırır. İki firma PostgreSQL web/pool/backup izolasyon testi CI kapısındadır. Süper Admin provisioning ekranı Faz 2B’de; legacy route taşıması, canlı WAL/PITR ayarı/ölçümü, production restore onayı ve canlı DNS/TLS/deployment sonraki paketlerde kalır.
