...
Faz 2B/2C uygulama notu: `apps/platform-admin` ayrı platform hostu, kimliği, oturumu, zorunlu TOTP MFA’sı, rol/izinleri ve 360° operasyon sayfalarıyla gerçek Platform DB’ye bağlıdır. UI komutları idempotent `PlatformAdminCommand` kuyruğuna yazar; `apps/provisioning-cli worker --once` ve `apps/tenant-ops-cli worker --once` mevcut gerçek DB/backup adapterlerini çalıştırır. `@tilbecore/tenant-runtime` ve `@tilbecore/tenant-web-runtime` request-local tenant bağlama, auditli SupportSession ve ayrı pool sınırını korur. İki firma PostgreSQL web/pool/backup izolasyon testi CI kapısındadır. Tam WebAuthn/passkey, legacy route taşıması, canlı WAL/PITR ayarı/ölçümü, production restore onayı ve canlı DNS/TLS/deployment sonraki paketlerde kalır.
