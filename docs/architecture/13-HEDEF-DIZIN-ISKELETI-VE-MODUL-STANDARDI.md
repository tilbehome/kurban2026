...
| `apps/tenant-ops-cli` | Tenant/ref doğrulamalı backup create/status/verify ve destructive olmayan restore plan/verify komutları | Faz 2C’de gerçek PostgreSQL araçları ve izolasyon testiyle oluşturuldu |
...
| `packages/tenant-web-runtime` | Platform tenant/custom-domain/SupportSession metadata adapteri ile doğrulanmış tenant request composition root’u | İş modülü route’u, platform private importu, global mutable tenant state veya secret |
