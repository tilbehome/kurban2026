...
Bu bölüm kullanıcı tarafından sonradan onaylanan profesyonel panel ve teknoloji gereksinimlerini izler. `REQ-001..REQ-068` ve `68/68` açıklaması değiştirilmez; aşağıdaki `PRO-*` kimlikleri ek ürün/işletim kapsamıdır. Mevcut support session, audit, feature flag, backup, notification ve observability kararları yeni modül gibi çoğaltılmaz; ilgili PRO kayıtlarında kabul kriteri olarak genişletilir.
...
| PRO-002 | Merkezi Onay Kutusu | Firma Admin / operasyon | Firma sahibi, ana muhasebe, operasyon yöneticisi | `onay.kutusu.goruntule`, `onay.talep.onayla`, `onay.talep.reddet` | Talep, karar, gerekçe, önce/sonra değer | Unit, route, E2E, concurrency | İndirim, iptal, borçlu teslim, yedinci hisse ve sıra değişikliği yetkisiz tamamlanamaz | Faz 5, Faz 6, Faz 8 |
...
| PRO-014 | Güncelleme ve migration ön kontrolü | Platform Süper Admin | Platform operatörü | `platform.migration.onkontrol` | Ön kontrol sonucu, blokaj, onay | Dry-run, backup, migration, health check | Yedek, tenant health, kapasite, sürüm ve rollback kontrolü geçmeden migration uygulanmaz | Faz 2C, Faz 15 |
...
3. Kullanıcı tarafından onaylanan profesyonel ürün ve işletim gereksinimleri: Bu dosyadaki `PRO-001..PRO-029` satırlarıdır. Bunlar `REQ-001..REQ-068` sayımını değiştirmez ve ilgili fazlarda ayrı kabul kanıtı ister.
