...
- Placeholder/yakında sayfaları tamamlanmış özellik sayılmaz ve üretim/canlı menüsünde kullanıcıya sunulmaz; bu hedef `PLANNED` durumundadır.
...
| REQ-004 | Her gerçek hissedar ayrı müşteri kartı | Müşteri | `Musteri` | Ortak telefon olabilir, kart ayrı | Müşteri oluştur | Müşteri | Mobil müşteri | Müşteri oluştur | Yeni kart | Mükerrer uyarı | Var | Yok | Aynı telefonla iki kart | `IMPLEMENTED_UNVERIFIED` | Faz 3 |
| REQ-005 | Ailede ortak telefon | Müşteri | `telefon` index | Telefon unique değildir | Müşteri oluştur/ara | Uyarı | Uyarı | Müşteri | Kayda izin | Güçlü uyarı | Var | Yok | Ortak telefon kabul | `IMPLEMENTED_UNVERIFIED` | Faz 3 |
| REQ-006 | Mükerrer müşteri uyarısı | Müşteri | arama read-model | Engelleme değil uyarı | Müşteri oluştur | Uyarı paneli | Uyarı | Müşteri | Benzerleri göster | Farklı kişi seç | Var | Yok | Aynı isim uyarısı | `IMPLEMENTED_UNVERIFIED` | Faz 3 |
| REQ-007 | Sezonlar arası müşteri geçmişi | Sezon/Müşteri | `Sezon`, ilişki | Müşteri kimliği kalır, cari sezonlanır | Sezon geçişi | Geçmiş | Özet | Yönetici | Geçmiş görüntü | Arşiv | Var | Yok | Önceki sezon görünür | `IMPLEMENTED_UNVERIFIED` | Faz 8 |
| REQ-008 | Sezon bazlı cari | Finans | `Cari`, `Sezon` | Borç/alacak sezon bazlı | Cari servis | Cari | Tahsilat | Finans | Sezon bakiyesi | Kapanış | Var | Yok | Sezon mutabakat | `IMPLEMENTING` | Faz 10 |
| REQ-009 | Tedarikçi ve alış faturası | Tedarik | `Tedarikci`, `AlisFaturasi` | Hayvan maliyeti belgeli | Alış kaydı | Tedarik | Yok | Tedarik | Fatura gir | İptal/ters | Var | Yok | Alış raporu | `IMPLEMENTED_UNVERIFIED` | Faz 4 |
| REQ-010 | Hayvan başı gerçek alış bedeli | Hayvan/Finans | Hayvan maliyet | Karlılık gerçek maliyetle | Hayvan maliyet | Hayvan kartı | Yok | Finans | Maliyet bağla | Düzeltme | Var | Yok | Karlılık hesap | `IMPLEMENTED_UNVERIFIED` | Faz 4 |
| REQ-011 | Küpe ve kurban no ayrımı | Hayvan | `kupeNo`, `kesimSirasi` | Kimlik ve operasyon no ayrıdır | Hayvan servis | Hayvan kartı | Kesim | Hayvan | Ayrı göster | No değişimi | Var | Yok | İki no görünür | `IMPLEMENTED_UNVERIFIED` | Faz 4 |
| REQ-012 | Kurban no ve değişebilir operasyon sırası | Kesim | `kesimSirasi`, `operasyonSira` | Operasyon sırası değişebilir | TV sıra | TV kontrol | Mobil kesim | Kesim sorumlusu | Sıra değiştir | Acil durum | Var | Kısmi | Sıra değişimi | `IMPLEMENTED_UNVERIFIED` | Faz 11 |
| REQ-013 | Tartım geçmişi | Tartım | Tartım kayıtları | Her tartım geçmişe yazılır | Tartım servis | Tartım | Tartım keypad | Tartım | Tartım ekle | Düzeltme | Var | Yok | Geçmiş görünür | `IMPLEMENTED_UNVERIFIED` | Faz 11 |
...
| REQ-019 | Rezervasyon, kapora ve son tarih | Satış/Tahsilat | rezervasyon, kapora, vade | Kaporasız kayıt yalnız süreli rezervasyon; pozitif tahsilatta kesin satış/alacak | Rezervasyon/kesinleştirme | Satış | Mobil | Satış ve tahsilat | Pozitif kaporayla kesinleştir | Sıfır tutarı reddet | Var | Kısmi | Rezervasyon finans üretmez | `IMPLEMENTING` | Faz 5–6 |
| REQ-020 | Rezervasyon süre sonu | Satış | durum, vade | Ödemesiz rezervasyon süre sonu olayıyla kapanır; oluşmamış satışa reversal yok | Worker/use-case | Liste | Uyarı | Yetkili | Süre sonu ve envantere açma | İdempotent tekrar | Var | Yok | Finans/vekâlet oluşmaz | `PLANNED` | Faz 5 |
...
| REQ-023 | Satılmamış işletme hissesi | Hisse | işletme envanteri durumu | Sahte müşteri/satış/gelir/alacak/vekâlet yok; dinî uygunluk çözümü açık karar | Envanter/engel use-case | Hisse 360 | Saha engeli | Yetkili görüntüleme | Envanterde açık tut | Kesim öncesi karar engeli | Zorunlu | Yok | Sahte kayıt oluşmaz | `PLANNED` | Faz 5/8 |
...
| REQ-029 | Hisse bazlı vekâlet | Vekâlet | `ProxyGrant.shareId` | Her hisse ayrı kontrol | Vekâlet servisi | Vekâlet | Mobil | Vekâlet | Yükle | İptal/yenile | Var | Var | Hisse bazlı belge | `IMPLEMENTED_UNVERIFIED` | Faz 7 |
| REQ-030 | Çoklu vekâlet veren | Vekâlet | `ProxyGrantor` | Bir hisse için çoklu veren | Vekâlet servisi | Form | Mobil | Vekâlet | Veren ekle | Eksik | Var | Var | Çoklu listelenir | `IMPLEMENTED_UNVERIFIED` | Faz 7 |
| REQ-031 | WhatsApp/telefon/sözlü vekâlet | Vekâlet | yöntem/kanal | Yöntem kontrollü kodla kayıtlı; gerçek kanal sağlayıcısı kapsam dışı | Vekâlet servisi | Form | Mobil | Vekâlet | Yöntem seç | Kanıt eksik | Var | Var | Yöntem görünür | `IMPLEMENTED_UNVERIFIED` | Faz 7 |
| REQ-032 | A4 QR kesim ve teslim belgesi | Belge | belge snapshot ve QR | QR süreli, iptal edilebilir ve tüketilebilir | Belge servisi | Belge görünümü | Mobil QR | Belge | Üret | Yenile | Var | Var | QR doğrula; A4/fiziksel baskı kabulü `NOT_RUN` | `IMPLEMENTED_UNVERIFIED` | Faz 7/12 |
| REQ-033 | Kayıp belge yenileme | Belge | belge versiyon/iptal bağı | Yeni belge eskiyi auditler | Belge servisi | Yenile | Mobil | Yönetici | Yenile | Eski iptal | Zorunlu | Var | Versiyon görünür | `IMPLEMENTED_UNVERIFIED` | Faz 7 |
| REQ-034 | Borçlu belge override | Teslim/Belge | onay ve gerekçe | Borçlu teslim özel onay | Teslim servisi | Teslim | QR | Yönetici | Onayla | Reddet | Zorunlu | Var | Neden zorunlu | `IMPLEMENTED_UNVERIFIED` | Faz 10/12 |
| REQ-035 | Kesim sırası ve acil değişiklik | Kesim | operasyon sıra ve geçiş geçmişi | Acil sıra değişimi auditli | Kesim servisi | Komuta merkezi | Kesim mobil | Kesim sorumlusu | Sıra değiş | Çakışma | Zorunlu | Var | Read-model güncellenir | `IMPLEMENTED_UNVERIFIED` | Faz 8 |
| REQ-036 | TV ve mobil takip | TV/Takip | PII'siz read-model | PII minimize | Takip servisi | TV/komuta | Müşteri PWA | Public/kısıtlı | Takip | Süre/iptal | Var | Var | Yetkili token sonucu; fiziksel ekran kabulü `NOT_RUN` | `IMPLEMENTED_UNVERIFIED` | Faz 8/10/12 |
| REQ-037 | Gerçek paket tartımı | Paket | tartım ve düzeltme geçmişi | Hisse gerçek kg alır | Tartım servisi | Paket | Mobil | Paketleme | Tart | Düzelt/iptal | Var | Var | Paket kg görünür | `IMPLEMENTED_UNVERIFIED` | Faz 9 |
| REQ-038 | Et parçalarının eşit dağıtımı | Paket | parça dağıtımı | Yedi hisseye parça ve ağırlık dağıtımı kayıtlı | Paket servisi | Paket ekranı | Mobil | Paketleme | Dağıt | Eksik parça | Var | Var | Paket bileşenleri; saha adalet kabulü `NOT_RUN` | `IMPLEMENTED_UNVERIFIED` | Faz 9/12 |
| REQ-039 | Alt kg iadesi | Finans/Paket | kg fark ayarlaması | Alt kg farkı onay bekleyen ayarlama üretir; otomatik ledger kaydı yok | Ayarlama servisi | Rapor | Bildirim | Finans | İade hesapla | Yönetici onay | Var | Var | `pending_approval` ayarlaması | `IMPLEMENTED_UNVERIFIED` | Faz 9/12 |
| REQ-040 | Üst kg ek ücret olmaması | Finans/Paket | kg fark | Üst kg ekstra borç doğurmaz | Ayarlama servisi | Rapor | Yok | Finans | Sıfır ek ücret | Bilgi | Var | Var | Ek borç yok | `IMPLEMENTED_UNVERIFIED` | Faz 9 |
| REQ-041 | Çiftlik/adres teslim | Teslim | teslim tipi ve hedef snapshot | Teslim noktası kayıtlı | Teslim servisi | Teslim | Mobil | Teslim | Tip seç | Adres eksik | Var | Var | Tip raporu | `IMPLEMENTED_UNVERIFIED` | Faz 10 |
| REQ-042 | Tek seferlik teslim | Teslim | teslim kapatma/geri alma geçmişi | Tek QR kontrollü kapanır | Teslim servisi | Teslim | QR | Teslimat | Kapat | Tekrar/geri alma | Var | Var | İkinci teslim engeli | `IMPLEMENTED_UNVERIFIED` | Faz 10 |
| REQ-043 | Yedi teslim olmadan hayvan kapanmaz | Teslim/Hayvan | kapanış kontrolü | Tüm hisseler teslim olmalı | Teslim servisi | Hayvan | Mobil | Yönetici | Kapat | Eksik hisse | Var | Var | Eksikte kapanmaz | `IMPLEMENTED_UNVERIFIED` | Faz 10 |
| REQ-044 | Mobil PWA | PWA | tenant/sezon/kullanıcı/oturum/cihaz bağlı kuyruk | İzinli offline akış ve fail-closed kritik işlem sınırı | Offline runtime | `/saha` | Mobil | Rol | Kuyruğa al | Sync/poison/conflict | Var | Var | Runtime ve saha bağı; fiziksel PWA kabulü `NOT_RUN` | `IMPLEMENTED_UNVERIFIED` | Faz 10/12 |
...
| PRO-002 | Merkezi Onay Kutusu | Firma Admin / operasyon | Firma sahibi, ana muhasebe, operasyon yöneticisi | `onay.kutusu.goruntule`, `onay.talep.onayla`, `onay.talep.reddet` | Talep, karar, gerekçe, önce/sonra değer | Unit, route, E2E, concurrency | İndirim, ödemeli satış iptali, teslimat geri alma, kasa kapatma, yetki değişikliği, toplu işlem, borçlu teslim ve sıra değişikliği yeniden doğrulama veya ikinci yetkili onayı olmadan tamamlanamaz; satılmamış işletme hissesi için sahte satış/kişi onayı üretilmez | Faz 5, Faz 6, Faz 8 |
...
| PRO-012 | Firma kurulum/provisioning sihirbazı | Platform Süper Admin | Platform operatörü | `platform.firma.provision` | Kurulum adımı, hata, rollback, iki ayrı admin daveti | Provisioning integration, rollback, tenant isolation, IAM negatifleri | Firma DB, modül hakları ve başlangıç ayarları kanıtlı kurulur veya geri alınır; ilk admin dışında en az bir aktif yedek Firma Admin olmadan aktivasyon tamamlanmaz (`PLANNED`) | Faz 2B, Faz 2C |
...
| PRO-030 | `IMPLEMENTED_UNVERIFIED`: Sezon durum makinesi ve arşiv kapanış kapısı | Firma Admin / operasyon | Firma sahibi, operasyon sorumlusu, muhasebe sorumlusu | `sezon.durum.goruntule`, `sezon.durum.gecir` | Durum geçişi, gerekçe, önce/sonra değer, kilitlenen işlem, kapanış snapshot'ı | State machine unit, route, yetki, E2E, geri dönüş testi | Sezon geçişleri ve arşiv öncesi operasyon/finans hazır olma kontrolü kodlanmıştır; Faz 12 doğrulaması `NOT_RUN` | Faz 2D, Faz 11, Faz 12 |
...
| PRO-032 | `IMPLEMENTED_UNVERIFIED`: Operasyonel tutarlılık ve sezon kapanış denetimleri | Firma Admin / operasyon kontrol | Firma sahibi, operasyon ve muhasebe sorumluları | `denetim.tutarlilik.calistir`, `denetim.istisna.yonet` | Denetim çalışması, bulgu, atama, çözüm, kapatma | Rules engine unit, integration, E2E, rapor testi | Kesim/teslim istisnaları, ledger mutabakatı ve sezon arşiv hazır olma kontrolü kodlanmıştır; listedeki bütün kural ve E2E kabulleri Faz 12'de doğrulanacaktır | Faz 8, Faz 10, Faz 11, Faz 12 |
| PRO-033 | `IMPLEMENTED_UNVERIFIED`: Güvenli çevrimdışı işlem kuyruğu runtime ve `/saha` bağı | Mobil PWA / operasyon | Saha kullanıcısı, operasyon sorumlusu | `offline.kuyruk.yaz`, `offline.kuyruk.esitle` | Kuyruk kaydı, tenant/sezon/kullanıcı/oturum/cihaz bağı, idempotency anahtarı, senkronizasyon sonucu, çakışma kararı | Offline queue unit, conflict, idempotency, mobile E2E | Bağlantı kesintisinde yalnız izin verilen işlemler güvenli kuyruğa alınır; kritik finans işlemleri sessiz tamamlanmış sayılmaz; fiziksel mobil E2E `NOT_RUN` | Faz 10, Faz 12 |
| PRO-034 | `IMPLEMENTED_UNVERIFIED`: Acil durum / yalnızca okuma modu | Firma Admin / saha PWA / TV | Firma sahibi, operasyon sorumlusu, platform operatörü | `operasyon.readonly.ac`, `operasyon.readonly.kapat` | Mod değişikliği, gerekçe, etkilenen ekranlar | Feature flag, E2E, yetki, çıktı smoke | Normal, yalnız okuma ve acil durdurma modları ile riskli yazma engeli kodlanmıştır; saha kabulü `NOT_RUN` | Faz 8, Faz 10, Faz 12 |
| PRO-035 | `IMPLEMENTED_UNVERIFIED`: Donanım adaptör sözleşmeleri | Firma Admin / saha cihazları | Firma admini, teknik operatör, saha personeli | `donanim.adapter.yonet`, `donanim.okuma.kaydet` | Cihaz okuma/yazdırma olayı, hata | Adapter contract, mock cihaz, E2E smoke | Terazi ve etiket/yazdırma bağı domain kodundan portlarla ayrılmıştır; gerçek cihaz adaptörü ve fiziksel kabul `NOT_RUN` | Faz 9, Faz 12 |
...
Faz 1–12 staging kabul adayı eşlemesi (`dce7d539122c1ae263cec566d18e907a5a63b0f1`): `PRO-022` için OpenTelemetry Node SDK/collector ve requestId/traceId korelasyonu; `PRO-023` için 13 Playwright projesi ve sentetik kritik akış senaryoları; `PRO-024`/`PRO-025` için axe, klavye, focus, yüzde 200 zoom/reflow ve form-hata ilişkisi kontrolleri; `PRO-026` için fiziksel passkey kabul runbook'u; `PRO-031` için iki sentetik tenant fixture'lı staging paketi ve prova profili; `PRO-033` için güvenli offline runtime/unit testleri uygulanmıştır. Gerçek staging, browser/axe, fiziksel cihaz/passkey, yük ve PITR koşuları tamamlanmamıştır; ilgili kanıtlar `BLOCKED` veya `NOT_RUN` olarak tutulur.
...
| `REQ-035..REQ-038`, `REQ-041..REQ-043` | `slaughter-packaging-delivery` | `packages/workflow-engine`, `apps/tenant-web`, hedef tercih `apps/field-pwa` (`DEC-001`) | Kesim, tartım, paketleme ve teslim durum makineleri. |
| `REQ-044..REQ-045` | `mobile-pwa-task-ui` | `packages/ui`, `packages/i18n`, hedef tercih `apps/field-pwa` (`DEC-001`) | Mobil PWA yüksek stres görev ekranları. |
