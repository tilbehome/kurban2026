...
export default defineConfig({
  ...
  projects: [
    ...
    { name: "android-phone", testMatch: /(?:public-surfaces|field-pwa(?:-offline)?)\.spec\.ts/, use: { ...devices["Pixel 7"] } },
    ...
    { name: "field-pwa", testMatch: /field-pwa(?:-offline)?\.spec\.ts/, use: { ...devices["Pixel 7"], baseURL: tenantBaseUrl } },
...
function acceptanceUrl(name: string, fallback: string): string {
  ...
  if (enabled) {
    ...
    const local = url.hostname === "localhost" || url.hostname === "127.0.0.1" || url.hostname === "::1" || url.hostname === "tilbecore.test" || url.hostname.endsWith(".tilbecore.test");
