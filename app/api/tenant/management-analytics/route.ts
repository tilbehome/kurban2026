...
const Body = z.discriminatedUnion("action", [
  ...
  z.object({ action: z.literal("report"), reportKey: z.enum(["sales-occupancy", "operations-bottleneck", "delivery-cold-storage", "audit-exceptions"]), filters: z.record(z.string(), z.string().optional()).optional() }),
  z.object({ action: z.literal("report-builder"), reportKey: z.enum(["sales-occupancy", "operations-bottleneck", "delivery-cold-storage", "audit-exceptions"]), filters: z.record(z.string(), z.string().optional()).optional(), dimensions: z.array(z.string().min(1)).max(6).optional(), measures: z.array(z.string().min(1)).max(6).optional() }),
